...
class UsageServiceTest {
    ...
    @BeforeEach
    void setUp() {
        when(balanceCache.opsForValue()).thenReturn(valueOps);
    ...
    void getBalance_cacheHit() throws Exception {
        ...
        when(valueOps.get("omobio:balance:t1:conn-A")).thenReturn(cached);
    ...
    void getBalance_cacheMiss_callsProviderAndCaches() throws Exception {
        when(valueOps.get("omobio:balance:t1:conn-A")).thenReturn(null);
        when(providerRegistry.getProvider("t1")).thenReturn(provider);
        when(provider.fetchBalance("conn-A")).thenReturn(balance(500));
        ...
        verify(valueOps).set(eq("omobio:balance:t1:conn-A"), eq(balance(500)), any());
    }
    ...
    void invalidateCache() {
        ...
        verify(balanceCache).delete("omobio:balance:t1:conn-A");
    }
