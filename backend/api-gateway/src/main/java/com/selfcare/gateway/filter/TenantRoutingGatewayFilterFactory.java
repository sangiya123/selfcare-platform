...
public class TenantRoutingGatewayFilterFactory
    ...
    private static final String CORRELATION_ID_PREFIX = "selfcare-";
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            // ----------------------------------------------------------------
            // The gateway is the first trusted edge — X-Forwarded-For is set to
            // the resolved client IP (leftmost inbound proxy hop when present),
            // never inherited from an untrusted chain.
     ...
     * {@code SELFCARE_ENV} environment variable falling back to the
    ...
    private String resolveEnvironment(org.springframework.web.server.ServerWebExchange exchange) {
        String env = System.getenv("SELFCARE_ENV");
        ...
        if (host != null && !host.isBlank()) {
            ...
            // Detect environment from hostname patterns (e.g. stg-api.selfcare.com)
