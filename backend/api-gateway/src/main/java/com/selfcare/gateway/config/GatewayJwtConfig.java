 ...
 * {@code selfcare.security.jwt.jwks-uri}.  For local development a self-managed
 * RSA key pair is generated in-memory.
 *
 * <p>Configuration:
 * <ul>
 *   <li>{@code selfcare.security.jwt.issuer} &mdash; expected {@code iss} claim</li>
 *   <li>{@code selfcare.security.jwt.audience} &mdash; expected {@code aud} claim</li>
 *   <li>{@code selfcare.security.jwt.jwks-uri} &mdash; JWKS endpoint (optional in dev)</li>
 *   <li>{@code selfcare.security.jwt.local-key.enabled} &mdash; generate a local key pair (dev only)</li>
 * </ul>
 */
@Slf4j
@Configuration
public class GatewayJwtConfig {
    ...
    @Value("${selfcare.security.jwt.local-key.enabled:false}")
    private boolean localKeyEnabled;

    /**
     * Returns a {@link JwtService} configured for token validation only.
     *
     * <p>If {@code selfcare.security.jwt.local-key.enabled} is true (typical in
    ...
    public JwtService jwtService() {
        if (!localKeyEnabled) {
            log.warn("GatewayJwtConfig: local key not enabled. JwtService bean will be created "
                    ...
                    + "'selfcare.security.jwt.local-key.enabled=true' for local development.");
        }
