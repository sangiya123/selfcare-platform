...
public class GatewayErrorHandler implements WebExceptionHandler {
    ...
    public Mono<Void> handle(ServerWebExchange exchange, Throwable ex) {
        ...
        echoObservabilityHeaders(exchange, response, envelope.getCorrelationId());
    ...
    private ApiException buildEnvelope(ServerWebExchange exchange, Throwable ex) {
        ...
        if (ex instanceof ApiException apiEx) {
            // Preserve an already-constructed platform envelope verbatim
            return apiEx;
        }
    ...
    private String resolveCorrelationId(ServerWebExchange exchange) {
        ...
        return "selfcare-" + java.util.UUID.randomUUID().toString().substring(0, 8);
    }

    private void echoObservabilityHeaders(ServerWebExchange exchange,
                                          ServerHttpResponse response,
                                          String correlationId) {
        // Always echo the correlation ID so callers can trace the error
    ...
    private boolean isServiceNotFound(Throwable ex) {
        ...
        return msg.contains("not found") || msg.contains("no instances")
                || msg.contains("no available instances")
