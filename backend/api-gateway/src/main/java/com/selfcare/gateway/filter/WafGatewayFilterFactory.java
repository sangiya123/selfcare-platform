 ...
 * {@code selfcare.waf.enabled=false} (default), this filter is a no-op.
 *
...
public class WafGatewayFilterFactory
    ...
    @Value("${selfcare.waf.enabled:false}")
    private boolean enabled;

    @Value("${selfcare.waf.rules-file:classpath:waf-rules.yml}")
    private String rulesResource;

    @Value("${selfcare.waf.max-body-size:1048576}")
    ...
    private boolean containsPrivateIp(String fullUrl) {
        if (isPrivateIpUrl(fullUrl)) {
            return true;
        }
        try {
            String query = URI.create(fullUrl).getQuery();
            if (query == null) {
                return false;
            }
            // Inspect query params that carry target URLs (?url=, ?redirect=, ...)
            for (String pair : query.split("&")) {
                int eq = pair.indexOf('=');
                if (eq >= 0 && eq < pair.length() - 1
                        && isPrivateIpUrl(pair.substring(eq + 1))) {
                    return true;
                }
            }
