...
class WafGatewayFilterFactoryTest {
    ...
    private ServerWebExchange buildExchange(String method, String path, String query) {
        ...
        ServerHttpRequest request = MockServerHttpRequest.method(method, URI.create(fullPath)).build();
    ...
    void allowedMethods_passThrough() {
        for (String method : new String[]{"GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS", "HEAD"}) {
            ...
            ServerHttpRequest request = MockServerHttpRequest.method(method, URI.create("/api/v1/test")).build();
    ...
    void traceMethod_blocked() {
        ServerHttpRequest request = MockServerHttpRequest.method("TRACE", URI.create("/api/v1/test")).build();
        ServerWebExchange exchange = MockServerWebExchange.from(request);
    ...
    void connectMethod_blocked() {
        ServerHttpRequest request = MockServerHttpRequest.method("CONNECT", URI.create("/api/v1/test")).build();
        ServerWebExchange exchange = MockServerWebExchange.from(request);
        MockServerHttpResponse response = (MockServerHttpResponse) exchange.getResponse();
    ...
    void authRateLimit_blocksAfter10() {
        ...
        reset(chain);
        ServerHttpRequest request11 = MockServerHttpRequest
            .post(path)
            .remoteAddress(new java.net.InetSocketAddress("192.0.2.1", 0))
            .build();
    ...
    void differentIps_separateCounters() {
        ...
        ServerHttpRequest ip1Request = MockServerHttpRequest
            .post(path)
            .remoteAddress(new java.net.InetSocketAddress("192.0.2.1", 0))
            .build();
        ...
        reset(chain);
        when(chain.filter(any())).thenReturn(Mono.empty());
        ServerHttpRequest ip2Request = MockServerHttpRequest
            .post(path)
