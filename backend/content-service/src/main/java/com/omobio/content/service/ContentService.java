...
public class ContentService {
    ...
    public PaginationResponse<Article> listArticles(
        ...
        String cacheKey = "omobio:content:articles:" + tenantId + ":" + category + ":" + tag + ":" + pagination.getPage();
    ...
    public Article getArticle(String slug) {
        String tenantId = TenantContext.get().getTenantId();
        String cacheKey = "omobio:content:article:" + tenantId + ":" + slug;
        Object cached = cache.opsForValue().get(cacheKey);
    ...
    public List<Banner> getBanners(String position) {
        String tenantId = TenantContext.get().getTenantId();
        String cacheKey = "omobio:content:banners:" + tenantId + ":" + position;
        Object cached = cache.opsForValue().get(cacheKey);
    ...
    private void invalidateArticleCache(String tenantId) {
        try {
            var keys = cache.keys("omobio:content:article:" + tenantId + ":*");
            if (keys != null && !keys.isEmpty()) cache.delete(keys);
            var pageKeys = cache.keys("omobio:content:articles:" + tenantId + ":*");
    ...
    private void invalidateFaqCache(String tenantId) {
        try {
            var keys = cache.keys("omobio:content:faqs:" + tenantId + ":*");
            if (keys != null && !keys.isEmpty()) cache.delete(keys);
        } catch (Exception e) {
    ...
    private void invalidateBannerCache(String tenantId) {
        try {
            var keys = cache.keys("omobio:content:banners:" + tenantId + ":*");
            if (keys != null && !keys.isEmpty()) cache.delete(keys);
        } catch (Exception e) {
