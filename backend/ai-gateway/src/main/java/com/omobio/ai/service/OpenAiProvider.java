 ...
 *
 * Model: gpt-4o-mini by default (configurable via omobio.ai.providers.openai.model).
 */
@Slf4j
...
public class OpenAiProvider implements LlmProvider {
    ...
    @Value("${omobio.ai.providers.openai.api-key:}")
    private String defaultApiKey;

    @Value("${omobio.ai.providers.openai.model:gpt-4o-mini}")
    private String defaultModel;

    @Value("${omobio.ai.providers.openai.max-tokens:4096}")
    private int defaultMaxTokens;

    @Value("${omobio.ai.providers.openai.timeout-seconds:60}")
    ...
    public AIResponse chat(ChatRequest request) {
        ...
        String model = request.getExtraParams() != null &&
                request.getExtraParams().get("model") instanceof String s ? s : defaultModel;
    ...
    private String resolveApiKey(String tenantId) {
        // In production: consult TenantConfigurationService for per-tenant API key
