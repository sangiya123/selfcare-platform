 ...
 *
 * Default model: gemini-1.5-pro (configurable via omobio.ai.providers.google.model).
...
public class GoogleAiProvider implements LlmProvider {
    ...
    @Value("${omobio.ai.providers.google.api-key:}")
    private String defaultApiKey;

    @Value("${omobio.ai.providers.google.model:gemini-1.5-pro}")
    private String defaultModel;

    @Value("${omobio.ai.providers.google.timeout-seconds:60}")
    ...
    public AIResponse chat(ChatRequest request) {
        ...
        String model = request.getExtraParams() != null && request.getExtraParams().get("model") instanceof String s
                ? s
                : defaultModel;
        String url = GOOGLE_AI_BASE + "/models/" + model + ":generateContent?key=" + apiKey;
        Map<String, Object> body = buildRequestBody(request);

        try {
            WebClient webClient = webClientBuilder
                    .baseUrl(GOOGLE_AI_BASE)
