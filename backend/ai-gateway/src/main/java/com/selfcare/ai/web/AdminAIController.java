...
public class AdminAIController {
    ...
    @Operation(summary = "Remove a knowledge chunk from the RAG store")
    public ResponseEntity<ApiResponse<Boolean>> deleteKnowledge(@PathVariable String id) {
        String tenantId = TenantContext.get().getTenantId();
        boolean removed = vectorEmbeddingService.deleteChunk(id, tenantId);
        if (!removed) {
            return ResponseEntity.notFound().build();
        }
        return ResponseEntity.ok(ApiResponse.of(true, TenantContext.get().getCorrelationId()));
    }
