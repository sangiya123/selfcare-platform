...
import com.selfcare.ai.service.ABTestingService;
...
class ABTestingServiceTest {
    ...
    void unknownExperiment() {
        when(valueOps.get("selfcare:ai:experiment:unknown")).thenReturn(null);
    ...
    void inactiveExperiment() {
        ABTestingService.Experiment exp = ABTestingService.Experiment.builder()
                ...
                .build();
        when(valueOps.get("selfcare:ai:experiment:exp-1")).thenReturn(exp);
    ...
    void cachedAssignment() {
        ...
        when(valueOps.get("selfcare:ai:experiment:exp-1")).thenReturn(exp);
        when(valueOps.get("selfcare:ai:assignment:exp-1:user-1")).thenReturn("treatment");
    ...
    void deterministicAssignment() {
        ...
        when(valueOps.get("selfcare:ai:experiment:exp-1")).thenReturn(exp);
        when(valueOps.get("selfcare:ai:assignment:exp-1:user-1")).thenReturn(null);
    ...
    void resolveControlTemplate() {
        ...
        when(valueOps.get("selfcare:ai:experiment:prompt-default")).thenReturn(null);
    ...
    void saveExperiment() {
        ...
        verify(valueOps).set(eq("selfcare:ai:experiment:exp-1"),
