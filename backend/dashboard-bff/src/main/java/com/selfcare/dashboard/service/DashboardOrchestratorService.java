...
public class DashboardOrchestratorService {
    ...
    private List<WidgetExecution> resolveWidgets(List<String> requestedIds, String tenantId, String connectionId, String profileKey) {
        if (requestedIds != null && !requestedIds.isEmpty()) {
            // Specific widget IDs requested — unknown IDs are kept so they surface
            // as UNAVAILABLE in the response (provider=null handled in executeWidget).
            return requestedIds.stream()
                    .map(id -> new WidgetExecution(id, widgetProviders.get(id), null, tenantId, connectionId, profileKey))
                    .toList();
        }

        // Layout-driven: resolve the ordered widget set from DB-backed config
        // (the platform analogue of the legacy midend dashboard layout), falling
        // back to all providers available to this profile.
        List<String> layoutWidgets = layoutService.resolveLayout(tenantId, profileKey);
        if (layoutWidgets == null || layoutWidgets.isEmpty()) {
            layoutWidgets = widgetProviders.entrySet().stream()
                    .filter(e -> e.getValue().isAvailable(tenantId, connectionId, profileKey))
                    .map(Map.Entry::getKey)
                    .toList();
        }

        return layoutWidgets.stream()
                .filter(widgetProviders::containsKey)
                .filter(id -> widgetProviders.get(id).isAvailable(tenantId, connectionId, profileKey))
                .map(id -> new WidgetExecution(id, widgetProviders.get(id), null, tenantId, connectionId, profileKey))
                .toList();
    }
    ...
    private Mono<WidgetResult> executeWidget(WidgetExecution execution) {
        ...
        if (provider == null) {
            return Mono.just(WidgetResult.unavailable(widgetId, "Widget not available"));
        }
