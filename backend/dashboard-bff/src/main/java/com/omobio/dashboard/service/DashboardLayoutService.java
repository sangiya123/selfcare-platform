...
public class DashboardLayoutService {
    ...
    private static final String LAYOUT_CACHE_PREFIX = "omobio:dashboard:layout:";
    ...
    /**
     * Load layout from MongoDB layout_documents.
     * In production: query the layout_documents collection.
     * For now: returns null to trigger default fallback.
     */
    @SuppressWarnings("unchecked")
    private List<String> loadLayoutFromMongo(String tenantId, String profileKey) {
        // In production: MongoTemplate or ChatSessionRepository-style approach
        // LayoutDocument layout = mongoTemplate.findOne(
        //     Query.query(Criteria.where("tenantId").is(tenantId)
        //                   .and("profileKey").is(profileKey))),
        //     LayoutDocument.class);
        // return layout != null ? layout.getWidgetOrder() : null;
        return null;
    }
