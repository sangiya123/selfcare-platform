...
import com.selfcare.identity.repository.SessionRepository;
...
class CustomerSessionServiceTest {
    ...
    @BeforeEach
    void setUp() {
        // Only used by tests that exercise the Redis session cache
    ...
    void createSession() {
        ...
        verify(valueOps).set(anyString(), eq("u-1"), any(Duration.class));
    }
    ...
    void rotateRefreshToken_replayDetected() {
        Session session = activeSession("sess-1");
        session.setCurrentTokenHash("hash-current");
        // Store the hash of the token we will (incorrectly) reuse
        session.setPreviousTokenHash(hash("previous-refresh-token"));
        when(sessionRepository.findById("sess-1")).thenReturn(Optional.of(session));
        when(sessionRepository.save(any())).thenAnswer(inv -> inv.getArgument(0));
        when(sessionRepository.findByTokenFamilyId(session.getTokenFamilyId())).thenReturn(List.of(session));

        // Send "previous-refresh-token" whose hash equals the stored previous hash
    ...
    private String hash(String value) {
        try {
            ...
            return java.util.HexFormat.of().formatHex(md.digest(value.getBytes()));
        } catch (Exception e) {
