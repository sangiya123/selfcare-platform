package com.omobio.identity.security;

import com.omobio.identity.domain.Session;
import com.omobio.identity.domain.TenantTokenPolicy;
import com.omobio.identity.service.TenantTokenPolicyService;
 ...
 * The {@code omobio.security.jwt.*} env-driven values are the platform
...
public class JwtIssuer {
    ...
    public JwtIssuer(JwtService jwtService,
                     ...
                     @Value("${omobio.security.jwt.key-id:omobio-selfcare-1}") String keyId,
                     @Value("${omobio.security.jwt.access-token-seconds:86400}") long defaultAccessTokenSeconds,
                     @Value("${omobio.security.jwt.refresh-token-seconds:2592000}") long defaultRefreshTokenSeconds) {
     ...
     * @throws com.omobio.platform.common.web.UnauthorizedException if invalid
     */
    public com.omobio.platform.common.security.JwtService.JwtClaims validateRefreshToken(String token) {
        var claims = jwtService.validate(token);
        if (!"refresh".equals(claims.getTokenType())) {
            throw new com.omobio.platform.common.web.UnauthorizedException(
     ...
     * for ephemeral / dev environments; production must use {@code omobio.security.jwt.private-key-pem}.
     */
    @Configuration
    public static class JwtConfig {
        ...
        @Value("${omobio.security.jwt.access-token-seconds:86400}")
        private long accessTokenSeconds;

        @Value("${omobio.security.jwt.refresh-token-seconds:2592000}")
        private long refreshTokenSeconds;

        @Value("${omobio.security.jwt.key-id:omobio-selfcare-1}")
        private String keyId;

        @Value("${omobio.security.jwt.private-key-pem:}")
        ...
        private KeyPair resolveKeyPair() {
            ...
            log.warn("No RSA key configured — generating ephemeral key pair for kid={}. " +
                    "Set omobio.security.jwt.private-key-pem for production.", keyId);
