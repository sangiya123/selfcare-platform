...
class TenantConfigServiceTest {
    ...
    void get_cacheHit() {
        TenantConfig cached = TenantConfig.builder()
                ...
                .industry("TELCO")
                .build();
        when(valueOps.get("selfcare:tenant:config:dialog-lk")).thenReturn(cached);
    ...
    void get_cacheMiss() {
        when(valueOps.get("selfcare:tenant:config:dialog-lk")).thenReturn(null);
        ...
        verify(valueOps).set(eq("selfcare:tenant:config:dialog-lk"), eq(tenant), any(Duration.class));
    }

    @Test
    @DisplayName("get throws NotFoundException when tenant not in DB")
    void get_notFound() {
        when(valueOps.get("selfcare:tenant:config:does-not-exist")).thenReturn(null);
    ...
    void get_lowercaseNormalize() {
        ...
        verify(valueOps).get("selfcare:tenant:config:dialog-lk");
    }
    ...
    void create_conflict() {
        ...
        assertThatThrownBy(() -> service.create(input))
                .isInstanceOf(com.selfcare.platform.common.web.ConflictException.class);
    }
    ...
    void update_invalidatesCache() {
        ...
        verify(redisTemplate).delete("selfcare:tenant:config:dialog-lk");
    }
    ...
    void delete_invalidatesCache() {
        ...
        verify(redisTemplate).delete("selfcare:tenant:config:stale-tenant");
    }
}
