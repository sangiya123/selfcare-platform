...
public class LayoutDocument {
    ...
    /**
     * Service endpoint definitions (auth, billing, usage, payments, ai, ...).
     * Authored in the admin portal, compiled + signed into the manifest so apps
     * resolve SDK endpoint paths from config — never app releases (v6 rule #1).
     */
    private Map<String, Object> services;

    /**
     * Data-source catalog: maps dataSource ids ("balance.current") to service
     * endpoint configs ({service, endpoint, method, params, transform}).
     * Authored in the admin portal, compiled + signed into the manifest so the
     * app resolves every data binding from config — no per-feature code.
     */
