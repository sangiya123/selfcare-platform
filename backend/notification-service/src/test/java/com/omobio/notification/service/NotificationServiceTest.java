...
class NotificationServiceTest {
    ...
    void send_sms_success() {
        ...
        when(templateService.render(eq("welcome-sms"), anyMap(), eq("en")))
                .thenReturn(new TemplateService.RenderedTemplate(
                        null, "Welcome to Dialog!", null));

        // Provider lookup
        when(providerRegistry.getProvider("dialog-lk")).thenReturn(smsProvider);
        when(smsProvider.supports(Channel.SMS)).thenReturn(true);
        when(smsProvider.send(anyString(), anyString(), anyString(), anyMap()))
                .thenReturn(new SendResult(true, "msg-id-123", null));
    ...
    void send_email_success() {
        when(templateService.render(eq("welcome-email"), anyMap(), eq("en")))
                .thenReturn(new TemplateService.RenderedTemplate(
                        "Welcome to Dialog!", "Your account is ready.", null));

        when(providerRegistry.getProvider("dialog-lk")).thenReturn(emailProvider);
        when(emailProvider.supports(Channel.EMAIL)).thenReturn(true);
        when(emailProvider.send(anyString(), anyString(), anyString(), anyMap()))
                .thenReturn(new SendResult(true, "email-msg-id", null));
    ...
    void send_push_success() {
        ...
        when(pushProvider.supports(Channel.PUSH)).thenReturn(true);
        when(pushProvider.send(anyString(), anyString(), anyString(), anyMap()))
                .thenReturn(new SendResult(true, "push-id-456", null));
    ...
    @Test
    @DisplayName("send throws IllegalArgumentException for unsupported channel")
    void send_unsupportedChannel() {
        when(templateService.render(any(), any(), any())).thenReturn(
                new TemplateService.RenderedTemplate(null, "body", null));
        when(providerRegistry.getProvider("dialog-lk")).thenReturn(smsProvider);
        when(smsProvider.supports(Channel.TELEGRAM)).thenReturn(false);
    ...
    @Test
    @DisplayName("getRecord returns notification by ID")
    void getRecord_found() {
        Notification notification = Notification.builder()
                ...
                .status("SENT")
                .build();
        when(notificationRepository.findByNotificationId("notif-1"))
                .thenReturn(Optional.of(notification));

        Notification result = service.getRecord("notif-1");

        assertThat(result.getNotificationId()).isEqualTo("notif-1");
    }

    @Test
    @DisplayName("getRecord throws NotFoundException for unknown ID")
    void getRecord_notFound() {
        when(notificationRepository.findByNotificationId("unknown"))
                .thenReturn(Optional.empty());

        assertThatThrownBy(() -> service.getRecord("unknown"))
    ...
    void send_inApp_success() {
        when(templateService.render(any(), any(), any())).thenReturn(
                new TemplateService.RenderedTemplate(null, "You have a new message", null));
        ...
        when(inAppProvider.supports(Channel.IN_APP)).thenReturn(true);
        when(inAppProvider.send(anyString(), anyString(), anyString(), anyMap()))
                .thenReturn(new SendResult(true, "inapp-id", null));
