...
import com.selfcare.payment.repository.PaymentTransactionRepository;
...
public class PaymentService {
    ...
    /**
     * List the authenticated user's transaction history (newest first).
     */
    @Transactional(readOnly = true)
    public org.springframework.data.domain.Page<PaymentTransaction> listHistory(int page, int size) {
        String tenantId = TenantContext.get().getTenantId();
        String userId = TenantContext.get().getUserId();
        return transactionRepository.findByTenantIdAndUserIdOrderByCreatedAtDesc(
                tenantId,
                userId == null ? "" : userId,
