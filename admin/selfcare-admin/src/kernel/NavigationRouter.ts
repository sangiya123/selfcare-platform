/**
 * selfcare kernel (web) — Navigation & Deep-Link Router.
 *
 * Indexes the manifest's `navigation` config into a route table and resolves:
 * - NAVIGATE actions (server-driven) to registered routes
 * - incoming deep links (https / custom scheme) into { route, params }
 *
 * Renderer-agnostic: the host (react-router) injects a `navigate` function.
 * Mirrors the mobile kernel (`mobile/selfcare-app/src/navigation/NavigationRouter.ts`).
 */

import { WebAction, NavItem } from './types';

/** Injected imperative navigation (react-router navigate / history). */
...
export class NavigationRouter {
  ...
  /** Top-level items suitable for a tab bar / top menu. */
  getTabs(): NavItem[] {
    return this.navItems;
  }
  ...
  /** Dispatch a manifest action. Currently only NAVIGATE is consumed. */
  handleAction(action: WebAction): boolean {
  ...
  parseUrl(url: string): DeepLink | null {
    ...
    if (/^https?:$/.test(parsed.protocol) && parsed.hostname) {
      ...
      route = host ? `/${host}${pathname}` : pathname;
    }
