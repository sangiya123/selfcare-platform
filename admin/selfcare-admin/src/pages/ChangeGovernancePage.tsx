...
export default function ChangeGovernancePage() {
  ...
  // Load change approvals from the approval-service admin API
        ...
        tab === 'in_review'
          ...
          : tab === 'published'
            ...
            : tab === 'rolled_back'
              ? 'ROLLED_BACK'
              : undefined;
      const res = await api.approvals.list(statusParam ? { status: statusParam } : undefined);
      const list = (Array.isArray(res) ? res : (res as any)?.data ?? []) as ConfigChange[];
      if (Array.isArray((res as any)?.content)) {
        return (res as any).content as ConfigChange[];
      }
      return list;
    },
  });
  ...
  const submitForReview = useMutation({
    mutationFn: (id: string) => api.approvals.submit({ changeId: id }),
    onSuccess: () => {
  ...
  const approveChange = useMutation({
    mutationFn: ({ id, comments }: { id: string; comments: string }) =>
      api.approvals.approve(id, comments),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['changes'] });
      toast.success('Approved and queued for publish');
    },
    onError: (err: any) => toast.error(err.message || 'Failed to approve change'),
  });

  const rejectChange = useMutation({
    mutationFn: ({ id, comments }: { id: string; comments: string }) =>
      api.approvals.reject(id, comments),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['changes'] });
      toast.success('Change rejected');
    },
    onError: (err: any) => toast.error(err.message || 'Failed to reject change'),
  });

  const rollback = useMutation({
    mutationFn: ({ id, reason }: { id: string; reason: string }) =>
      api.approvals.cancel(id, reason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['changes'] });
      toast.success('Rolled back to previous version');
    },
    onError: (err: any) => toast.error(err.message || 'Failed to roll back change'),
  });
