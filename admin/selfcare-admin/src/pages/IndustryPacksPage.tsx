 ...
 *
 * Active clients are fetched dynamically from the API — no hardcoded lists.
 */
...
/**
 * Map a tenant's industry value to the matching industry pack key.
 * The set of recognised industry values is owned by the platform — no hardcoded
 * tenant IDs here.
 */
function industryToPackKey(industry: string | undefined): string | null {
  if (!industry) return null;
  const upper = industry.toUpperCase();
  if (upper === 'TELCO') return 'telco';
  if (upper === 'INSURANCE') return 'insurance';
  if (upper === 'TRAVEL') return 'travel';
  if (upper === 'BANKING') return 'banking';
  return null;
}
...
export default function IndustryPacksPage() {
  ...
  // Derive active clients per pack from the tenant list returned by the API.
  // No hardcoded tenant IDs in this page.
  const clientsByPack = useMemo(() => {
    const grouped: Record<string, Array<{ tenantId: string; name?: string }>> = {};
    for (const t of allTenants) {
      const packKey = industryToPackKey(t.industry);
      if (!packKey) continue;
      if (!grouped[packKey]) grouped[packKey] = [];
      grouped[packKey].push({ tenantId: t.tenantId, name: t.name });
    }
    return grouped;
  }, [allTenants]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {PACKS.map((pack) => (
          <PackCard
            key={pack.key}
            pack={pack}
            clients={clientsByPack[pack.key] || []}
          />
        ))}
      </div>
    </div>
...
function PackCard({
  ...
  pack: IndustryPack;
  clients: Array<{ tenantId: string; name?: string }>;
}) {
  return (
    <Card>
      <CardHeader>
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          {clients.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-1">
              {clients.map((c) => (
                <Badge key={c.tenantId} variant="outline" className="font-mono text-xs">
                  {c.name || c.tenantId}
                </Badge>
              ))}
            </div>
