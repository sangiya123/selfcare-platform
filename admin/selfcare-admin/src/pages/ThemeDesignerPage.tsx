...
export default function ThemeDesignerPage() {
  ...
  const { data: themeData, isLoading } = useQuery({
    queryKey: ['theme', activeTenantId],
    queryFn: () => api.themes.list(activeTenantId!),
    enabled: !!activeTenantId,
  });

  // Initialize tokens from API or defaults
  ...
  // Update tokens when API data loads
  useEffect(() => {
    if (themeData && themeData.length > 0 && !dirty) {
      setTokens(themeData[0]);
    }
  }, [themeData, dirty]);
  ...
  // Mutation for saving theme
  const saveMutation = useMutation({
    mutationFn: (data: any) => api.themes.save(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['theme', activeTenantId] });
      toast.success('Theme saved successfully');
      setDirty(false);
    },
    onError: (err: any) => toast.error(err.message || 'Failed to save theme'),
  });
