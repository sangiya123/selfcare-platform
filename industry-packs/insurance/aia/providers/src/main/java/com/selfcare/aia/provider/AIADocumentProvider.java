...
import com.selfcare.platform.common.adapter.DocumentProvider;
...
public class AIADocumentProvider implements ApiAdapter, DocumentProvider {
    ...
    private final Map<String, com.selfcare.aia.provider.AIAInsuranceProvider.Config> configCache = new ConcurrentHashMap<>();
    ...
    private com.selfcare.aia.provider.AIAInsuranceProvider.Config loadConfig(String tenantId) {
        return configCache.computeIfAbsent(tenantId, this::resolveConfig);
    }

    private com.selfcare.aia.provider.AIAInsuranceProvider.Config resolveConfig(String tenantId) {
        return tenantConfig.getIntegration(tenantId, INTEGRATION_TYPE)
                .filter(c -> "ACTIVE".equalsIgnoreCase(c.getStatus()))
                .map(c -> new com.selfcare.aia.provider.AIAInsuranceProvider.Config(
                        tenantId,
                        c.getBaseUrl(),
                        ...
                        c.getMetadata()))
                .orElseGet(() -> com.selfcare.aia.provider.AIAInsuranceProvider.Config.empty(tenantId));
    }
    ...
    public UploadResult uploadDocument(String tenantId, String customerId, String fileName,
        ...
        com.selfcare.aia.provider.AIAInsuranceProvider.Config cfg = loadConfig(tenantId);
        if (!cfg.isValid()) {
