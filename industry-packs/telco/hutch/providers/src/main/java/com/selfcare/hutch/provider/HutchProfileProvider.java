...
/**
 * Hutch Profile Provider — fetches subscriber profile from Hutch BSS.
 *
 * <p>Implements the canonical {@link ApiAdapter} contract. Per-tenant config
 * (BSS base URL, API key) loaded from MongoDB via {@link HutchHttpClient}.
 * Configure via Selfcare Studio admin: Integrations &gt; Hutch BSS.</p>
 */
@Slf4j
@Component
@RegisterAdapter(tenantId = "hutch-lk", providerInterface = ApiAdapter.class)
@RequiredArgsConstructor
public class HutchProfileProvider implements ApiAdapter {
    ...
    public SubscriberProfile getProfile(String tenantId, String connectionId) {
        ...
        if (!cfg.isValid()) {
            log.warn("No active Hutch BSS integration for tenant={}", tenantId);
            return null;
        }
        ...
        try {
            ...
            log.error("Hutch profile fetch failed for connection={}: {}", connectionId, e.getMessage());
            return null;
        }
    }
