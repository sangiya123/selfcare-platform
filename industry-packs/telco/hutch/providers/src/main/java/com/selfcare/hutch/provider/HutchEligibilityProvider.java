...
/**
 * Hutch Eligibility Provider — checks product / plan eligibility from Hutch BSS.
 *
...
@Slf4j
@Component
@RegisterAdapter(tenantId = "hutch-lk", providerInterface = EligibilityProvider.class)
@RequiredArgsConstructor
public class HutchEligibilityProvider implements ApiAdapter, EligibilityProvider {
    ...
    public EligibilityResult checkEligibility(String tenantId, String connectionId,
        ...
        if (!cfg.isValid()) {
            ...
            return new EligibilityResult(false, connectionId, productCode, offerCode,
                    "NOT_FOUND", "No Hutch BSS integration configured", List.of(), Map.of());
        }
        ...
        try {
            Map<String, Object> resp = httpClient.post(tenantId, cfg,
                    "/api/v1/eligibility/check", body, null);
            if (resp == null) {
                return new EligibilityResult(false, connectionId, productCode, offerCode,
                        "NOT_FOUND", "Empty Hutch BSS response", List.of(), Map.of());
            }
            Object dataObj = resp.get("data");
            if (!(dataObj instanceof Map<?, ?> d)) {
                return new EligibilityResult(false, connectionId, productCode, offerCode,
                        "NOT_FOUND", "Unexpected Hutch BSS response shape", List.of(), Map.of());
            }
            ...
            log.error("Hutch checkEligibility failed for tenant={} connection={} product={}: {}",
    ...
    public List<EligibleProduct> getEligibleProducts(String tenantId, String connectionId) {
        ...
        String path = "/api/v1/subscribers/" + connectionId + "/eligible-products";
        try {
            ...
            log.error("Hutch getEligibleProducts failed for tenant={} connection={}: {}",
    ...
    public MigrationEligibility checkMigrationEligibility(String tenantId, String connectionId,
        ...
        if (!cfg.isValid()) {
            ...
            return new MigrationEligibility(connectionId, null, targetPlanCode, false,
                    "No Hutch BSS integration", null, null, null, List.of());
        }
        ...
        try {
            Map<String, Object> resp = httpClient.post(tenantId, cfg,
                    "/api/v1/plan/migration/eligibility", body, null);
            if (resp == null) {
                return new MigrationEligibility(connectionId, null, targetPlanCode, false,
                        "Empty Hutch BSS response", null, null, null, List.of());
            }
            Object dataObj = resp.get("data");
            if (!(dataObj instanceof Map<?, ?> d)) {
                return new MigrationEligibility(connectionId, null, targetPlanCode, false,
                        "Unexpected Hutch BSS response shape", null, null, null, List.of());
            }
            @SuppressWarnings("unchecked")
            Map<String, Object> m = (Map<String, Object>) d;
            boolean eligible = Boolean.TRUE.equals(m.get("eligible"));
            ...
            log.error("Hutch checkMigrationEligibility failed for tenant={} connection={}: {}",
                    tenantId, connectionId, e.getMessage());
            return new MigrationEligibility(connectionId, null, targetPlanCode, false,
    ...
    public FinancingEligibility checkFinancingEligibility(String tenantId, String connectionId,
        ...
        if (!cfg.isValid()) {
            ...
            return new FinancingEligibility(connectionId, deviceId, false,
                    "No Hutch BSS integration", null, 0, null, null);
        }

        var queryParams = Map.<String, Object>of("deviceId", deviceId);
        String path = "/api/v1/subscribers/" + connectionId + "/financing/eligibility";
        try {
            Map<String, Object> resp = httpClient.get(tenantId, cfg, path, queryParams, null);
            if (resp == null) {
                return new FinancingEligibility(connectionId, deviceId, false, "Empty Hutch BSS response",
                        null, 0, null, null);
            }
            Object dataObj = resp.get("data");
            if (!(dataObj instanceof Map<?, ?> d)) {
            ...
            log.error("Hutch checkFinancingEligibility failed for tenant={} connection={}: {}",
                    tenantId, connectionId, e.getMessage());
