...
public class DialogProductCatalogProvider implements ApiAdapter, ProductCatalogProvider {
    ...
    public List<RawProduct> fetchAllProducts(String tenantId) {
        ...
        CATEGORIES.forEach((category, lob) ->
                products.addAll(fetchCategory(tenantId, cfg, op, category, lob)));
    ...
    public RawProduct fetchProduct(String tenantId, String sourceProductId) {
        DialogHttpClient.DialogConfig cfg = httpClient.resolveConfig(tenantId, INTEGRATION_TYPE);
        DialogOperator op = new DialogOperator(cfg);
        if (!op.configured()) return null;
        String path = op.path("catalog.product", "/products/{id}",
                Map.of("id", sourceProductId));
        try {
            Map<String, Object> data = httpClient.get(tenantId, cfg.baseUrl(),
                    path, null, null, cfg.apiKey());
            if (data == null) return null;
            return mapToRawProduct(data, "MIXED", op);
    ...
    private List<RawProduct> fetchCategory(String tenantId, DialogHttpClient.DialogConfig cfg,
                                           DialogOperator op, String category, String lob) {
        try {
            String path = op.path("catalog." + category, "/products/{category}",
                    Map.of("category", category));
            Map<String, Object> data = httpClient.get(tenantId, cfg.baseUrl(),
                    path, null, null, cfg.apiKey());
            if (data == null) return Collections.emptyList();
            Object itemsObj = data.get(op.field("catalog.items", "items"));
            if (!(itemsObj instanceof List<?> items)) return Collections.emptyList();

            String lobValue = op.lob("catalog." + category, lob);
            var products = new ArrayList<RawProduct>();
            for (Object o : items) {
                if (o instanceof Map<?, ?> rawMap) {
                    products.add(mapToRawProduct((Map<String, Object>) rawMap, lobValue, op));
                }
            }
            return products;
        } catch (DialogApiException e) {
            log.warn("Failed to fetch Dialog catalog category {}: {}", category, e.getMessage());
            return Collections.emptyList();
        }
    }

    private RawProduct mapToRawProduct(Map<String, Object> item, String lob, DialogOperator op) {
        ...
        return RawProduct.builder()
                ...
                .price(price)
                .currency(item.get("currency") != null ? item.get("currency").toString() : op.currency())
