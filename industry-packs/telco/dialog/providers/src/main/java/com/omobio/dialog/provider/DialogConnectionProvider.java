...
public class DialogConnectionProvider implements ApiAdapter, ConnectionProvider {
    ...
    public List<Connection> getConnections(String tenantId, String primaryMsisdn) {
        ...
        if (!cfg.isValid()) {
            log.warn("Dialog BSS not configured for tenant={}, returning stub", tenantId);
            return List.of(
                    new Connection(primaryMsisdn, primaryMsisdn, "PRIMARY", "ACTIVE",
                            "DIALOG_PREPAID", null, "5G", null, null, null)
            );
        }

        // TODO: Replace with actual Dialog BSS API call to enumerate linked connections
        // GET /subscriber/{msisdn}/connections
        String path = "/subscriber/connections/" + primaryMsisdn;
        try {
            Map<String, Object> data = httpClient.get(tenantId, cfg.baseUrl(), path, null, null, cfg.apiKey());
            if (data == null) {
                return List.of(new Connection(primaryMsisdn, primaryMsisdn, "PRIMARY", "ACTIVE",
                        "DIALOG_PREPAID", null, "5G", null, null, null));
            }
            Object connectionsObj = data.get("connections");
    ...
    public Connection getConnection(String tenantId, String connectionId) {
        ...
        String path = "/subscriber/connections/" + connectionId;
        try {
            Map<String, Object> data = httpClient.get(tenantId, cfg.baseUrl(), path, null, null, cfg.apiKey());
    ...
    public ServiceStatus getServiceStatus(String tenantId, String connectionId) {
        ...
        String path = "/subscriber/status/" + connectionId;
        try {
            Map<String, Object> data = httpClient.get(tenantId, cfg.baseUrl(), path, null, null, cfg.apiKey());
            if (data == null) return null;
    ...
    public SimDetails getSimDetails(String tenantId, String connectionId) {
        ...
        String path = "/subscriber/sim/" + connectionId;
        try {
            Map<String, Object> data = httpClient.get(tenantId, cfg.baseUrl(), path, null, null, cfg.apiKey());
            if (data == null) return null;
