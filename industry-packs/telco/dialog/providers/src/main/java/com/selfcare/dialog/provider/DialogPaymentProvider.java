 ...
 * the platform-common adapter package so the {@code PaymentProviderRouter}
 * can dispatch transactions to this provider based on tenant. Also exposes
 * domain methods (recharge / payBill) for direct invocation.</p>
 *
...
public class DialogPaymentProvider implements ApiAdapter, PaymentProviderAdapter {
    ...
    public PaymentResult queryStatus(PaymentRequest tx) {
        ...
        return PaymentResult.builder()
                ...
                .providerReference(status.getProviderReference())
