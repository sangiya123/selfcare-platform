...
/**
 * Airtel Profile Provider — fetches subscriber profile from Airtel BSS.
 *
 * <p>Implements the canonical {@link ApiAdapter} contract. Per-tenant config
 * (BSS base URL, API key) loaded from MongoDB via {@link AirtelHttpClient}.
 * Configure via Selfcare Studio admin: Integrations &gt; Airtel BSS.</p>
 */
@Slf4j
@Component
@RegisterAdapter(tenantId = "airtel-lk", providerInterface = ApiAdapter.class)
@RequiredArgsConstructor
public class AirtelProfileProvider implements ApiAdapter {
    ...
    /**
     * Get subscriber profile by connection ID.
     */
    ...
    /**
     * Get subscriber profile by tenant + connection ID.
     */
    public SubscriberProfile getProfile(String tenantId, String connectionId) {
        ...
        if (!cfg.isValid()) {
            log.warn("No active Airtel BSS integration for tenant={}", tenantId);
            return null;
        }
        String path = "/subscriber/profile/" + connectionId;
        try {
            Map<String, Object> data = httpClient.get(tenantId, cfg, path, null, null);
            if (data == null) return null;
            return mapToProfile(data);
        } catch (AirtelApiException e) {
            log.error("Airtel profile fetch failed for connection={}: {}", connectionId, e.getMessage());
            return null;
        }
    }
